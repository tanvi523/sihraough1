...
import { getSignalTasks, updateSignalTaskStatus, createSignalTask, deleteSignalTask, clearAllSignalTasks } from '../services/api';
import {
  Radio,
  Search,
  Plus,
...
export const SignalMaintenancePage = () => {
  ...
  const [newTask, setNewTask] = useState({
    task_id: 'SMMS-' + Math.floor(1000 + Math.random() * 9000),
    asset_id: 'SIG-PUNE-' + Math.floor(1000 + Math.random() * 9000),
    city: 'Pune',
    track_section: 'Pune Jn-Shivajinagar',
    asset_type: 'Panel Interlocking',
    asset_condition: 'Fair',
    health_score: 55.0,
    issue_type: 'Cable Fault',
    severity: 'High',
  ...
  const fetchSignalTasks = async () => {
    try {
      ...
      const res = await getSignalTasks(params);
      setTasks(res.data);
    } catch (err) {
      console.warn('SMMS fetch note:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchSignalTasks();
  }, [search, selectedSection, selectedSeverity, selectedStatus, sortBy, sortOrder]);
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      await updateSignalTaskStatus(taskId, newStatus);
      showToast(`Signal Task ${taskId} status updated to ${newStatus}`, 'success');
      fetchSignalTasks();
    } catch (err) {
  ...
  const handleDelete = async (taskId) => {
    if (window.confirm(`Delete signal maintenance task ${taskId}?`)) {
      try {
        await deleteSignalTask(taskId);
        showToast(`Signal task ${taskId} removed successfully`, 'success');
        fetchSignalTasks();
      } catch (err) {
  ...
  const handleClearAll = async () => {
    if (window.confirm('Are you sure you want to clear ALL SMMS signal records?')) {
      try {
        await clearAllSignalTasks();
        showToast('All SMMS signal records cleared successfully', 'success');
        fetchSignalTasks();
      } catch (err) {
        showToast('Failed to clear SMMS: ' + err.message, 'error');
      }
    }
  };
  ...
  const handleCreateSubmit = async (e) => {
    e.preventDefault();
    try {
      await createSignalTask(newTask);
      showToast(`Signal task ${newTask.task_id} logged successfully!`, 'success');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="p-6 rounded-2xl bg-[#0E172A] border border-[#1F2E4D] shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">Signal Maintenance System (SMMS)</h2>
            <p className="text-xs text-slate-400">
              Quick Overview of Signal Maintenance Records
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span>Upload SMMS CSV</span>
          </button>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-semibold shadow-lg shadow-emerald-600/20 transition-all cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>Log Signal Fault</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-[#0E172A] border border-[#1F2E4D] shadow-xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search Asset, Task, Issue..."
            value={search}
        ...
        <select
          ...
          className="px-3 py-2 text-xs rounded-xl glass-input cursor-pointer"
        >
          <option value="" className="bg-[#0E172A]">All Statuses</option>
          <option value="Pending" className="bg-[#0E172A]">Pending Maintenance</option>
          <option value="Scheduled" className="bg-[#0E172A]">Scheduled in Block</option>
          <option value="In Progress" className="bg-[#0E172A]">In Progress</option>
          <option value="Completed" className="bg-[#0E172A]">Completed</option>
        </select>
      ...
      <div className="p-5 rounded-2xl bg-[#0E172A] border border-[#1F2E4D] shadow-xl overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            <h3 className="text-sm font-bold text-white">Signal Maintenance Assets</h3>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[#131D33] text-slate-400 font-mono uppercase text-[11px] border-b border-[#1F2E4D] select-none">
              <tr>
                ...
                <th onClick={() => handleSort('next_due_date')} className="py-3 px-4 cursor-pointer hover:text-white">
                  Due Date {renderSortIndicator('next_due_date')}
                </th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="9" className="text-center py-8 text-slate-400">
                    Loading signal assets...
                  </td>
                </tr>
              ) : tasks.length === 0 ? (
                <tr>
                  <td colSpan="9" className="text-center py-8 text-slate-400">
                    No signal maintenance records found.
                  </td>
                </tr>
              ) : (
                tasks.map((task) => (
                  <tr key={task.id || task.task_id} className="hover:bg-[#131D33]/40 transition-colors">
                    <td className="py-3 px-4">
                      <div className="font-mono font-bold text-emerald-400">{task.asset_id}</div>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    <option value="Panel Interlocking">Panel Interlocking</option>
                    <option value="Level Crossing Interlocking">Level Crossing Interlocking</option>
                    <option value="Axle Counter">Axle Counter</option>
                    <option value="Block Instrument">Block Instrument</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-300 mb-1">Severity</label>
                  <select
              ...
              <div className="pt-4 flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-xs font-bold text-white shadow-lg"
                >
                  Save Record
                </button>
              </div>
